...
router.post('/tally/sync-collection', async (req, res) => {
  ...
  console.log("TALLY INTEGRATION: Initiating fee collection sync.");

  res.json({
    success: true,
    message: "Tally collection sync initiated for today's transactions (Queueing for retry/error handling).",
    status: "queued"
  });
});

// POST: Tally webhook for reconciliation confirmation with secret verification
...
router.post('/defaulters/auto-alert', async (req, res) => {
  ...
  res.status(200).json({
    success: true,
    message: `Daily reminders sent successfully to ${sentCount} defaulters via Arattai/WhatsApp.`,
    sentCount
  });
});
